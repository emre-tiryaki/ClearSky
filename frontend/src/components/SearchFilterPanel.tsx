...
export function SearchFilterPanel({
    ...
    const availableCategories = useMemo(() => {
        ...
        return {
            weight: buildCategoryOptions(WEIGHT_CATEGORY_CODES.filter(c => present.has(c))),
            type: buildCategoryOptions(TYPE_CATEGORY_CODES.filter(c => present.has(c))),
        };
    }, [flights]);
            ...
            {isOpen && (
                <div className="absolute right-0 top-0 h-full w-80 bg-white shadow-2xl z-1000 flex flex-col">
                    <div className="flex items-center justify-between px-4 py-3 border border-slate-200">
                        <h2 className="font-semibold text-slate-800">Search & Filter</h2>
                        <button
                            onClick={onToggle}
                    ...
                    <div className="flex-1 overflow-y-auto px-4 py-4 space-y-6">
                        <div className="relative">
                            <label className="text-sm font-medium text-slate-600">ICAO24 / Callsign</label>
                            <input 
                                type="text"
                                ...
                                <ul className="absolute left-0 right-0 mt-1 bg-white border border-slate-200 rounded shadow-lg max-h-56 overflow-y-auto z-10">
                                    {suggestions.map(flight => (
                                        <li key={flight.icao24}>
                                            <button
                                                onClick={() => handleSelectSuggestion(flight.icao24)}
                        ...
                        <div>
                            ...
                            <div className="mt-1 flex gap-2">
                                {(Object.keys(STATUS_LABELS) as FlightStatusFilter[]).map(status => (
                                    <button
                                        key={status}
                        ...
                        <div>
                            <label className="text-sm font-medium text-slate-600">Vertical Rate</label>
                            <div className="mt-1 flex flex-wrap gap-2">
                                {(Object.keys(VERTICAL_RATE_LABELS) as VerticalRateFilter[]).map(rate => (
                                    <button
                                        key={rate}
                                        onClick={() => update({verticalRate: rate})}
                                        className={`px-3 py-1.5 text-xs font-medium rounded border ${
                        ...
                        <div>
                            ...
                            <div className="mt-1 flex gap-2">
                                <input
                                    ...
                                    onChange={e => update({ altitudeMin: e.target.value === "" ? null : Number(e.target.value) })}
                                    placeholder="Min"
                                ...
                                <input
                                    ...
                                    onChange={e => update({ altitudeMax: e.target.value === "" ? null : Number(e.target.value) })}
                                    placeholder="Max"
                                    className="w-1/2 rounded border border-slate-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                                />
                            </div>
                        ...
                        <div>
                            <label className="text-sm font-medium text-slate-600">Speed (m/s)</label>
                            <div className="mt-1 flex gap-2">
                                <input
                                    type="number"
                                    value={filters.speedMin ?? ""}
                                    onChange={e => update({ speedMin: e.target.value === "" ? null : Number(e.target.value) })}
                                    placeholder="Min"
                                    className="w-1/2 rounded border border-slate-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                                />
                                <input
                                    ...
                                    onChange={e => update({ speedMax: e.target.value === "" ? null : Number(e.target.value) })}
                                    placeholder="Max"
                                    className="w-1/2 rounded border border-slate-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                                />
                            </div>
                        ...
                        {availableCategories.weight.length > 0 && (
                            <div>
                                <label className="text-sm font-medium text-slate-600">Weight Class</label>
                                <div className="mt-1 space-y-1">
                                    {availableCategories.weight.map(option => (
                                        <label key={option.code} className="flex items-center gap-2 text-sm text-slate-700">
                                            <input
                                                type="checkbox"
                                                checked={filters.selectedCategories?.has(option.code) ?? false}
                        ...
                        {availableCategories.type.length > 0 && (
                            <div>
                                <label className="text-sm font-medium text-slate-600">Aircraft Type</label>
                                <div className="mt-1 space-y-1">
