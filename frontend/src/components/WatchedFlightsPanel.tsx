...
function groupByAircraft(records: FlightRecord[]): UniqueAircraft[] {
    const map = new Map<string, { callsign: string | null; records: FlightRecord[] }>();
    for (const r of records) {
        if (!map.has(r.icao24)) map.set(r.icao24, { callsign: r.callsign, records: [] });
...
function closestRecord(records: FlightRecord[], icao24: string, targetMs: number): FlightRecord | null {
            ...
            {isOpen && (
                <div className="absolute left-0 top-0 h-full w-80 bg-white shadow-2xl z-1000 flex flex-col">
                    ...
                    <div className="flex items-center justify-between px-4 py-3 border-b border-slate-200">
                        <h2 className="font-semibold text-slate-800">Tracked Flights</h2>
                        <button
                            onClick={onToggle}
                    ...
                    <div className="flex-1 overflow-y-auto px-4 py-4 space-y-5">
                        ...
                        {watchedIcao24s.size > 0 && (
                            <div>
                                <label className="text-sm font-medium text-slate-600">Currently Watching</label>
                                <ul className="mt-1 space-y-1">
                                    {Array.from(watchedIcao24s).map(icao24 => (
                                        <li key={icao24} className="flex items-center justify-between rounded border border-green-200 bg-green-50 px-2 py-1.5">
                                            <span className="text-sm font-medium text-green-800">{icao24}</span>
                                            <button
                                                onClick={() => onStopWatching(icao24)}
                            ...
                            <div className="mt-1 space-y-2">
                                <div className="flex items-center gap-2">
                                    ...
                                    <input
                                        ...
                                        onChange={e => onRangeStartChange(new Date(e.target.value).getTime())}
                                ...
                                <div className="flex items-center gap-2">
                                    ...
                                    <input
                                        ...
                                        onChange={e => onRangeEndChange(new Date(e.target.value).getTime())}
                                        className="flex-1 rounded border border-slate-300 px-2 py-1 text-xs focus:outline-none focus:ring-2 focus:ring-blue-500"
                                    />
                                </div>
                                <button
                        ...
                        {records.length > 0 && (
                            <div>
                                <label className="text-sm font-medium text-slate-600">Current Moment</label>
                                ...
                                <div className="flex justify-between text-xs text-slate-400 mt-0.5">
                                    <span>{fmt(new Date(rangeStartMs).toISOString())}</span>
                                    <span>{fmt(new Date(rangeEndMs).toISOString())}</span>
                                </div>
                            </div>
                        )}
                        ...
                        <div>
                            <label className="text-sm font-medium text-slate-600">
                                Aircraft in Range
                                {aircraft.length > 0 && (
                                    <span className="ml-2 text-xs text-slate-400">({aircraft.length})</span>
                                )}
                            </label>
                            {loadingRecords && (
                            ...
                            {!loadingRecords && aircraft.length === 0 && (
                                <p className="mt-1 text-xs text-slate-400">No records in this range.</p>
                            )}
                            <ul className="mt-1 space-y-1">
                                {aircraft.map(a => (
                                    <li key={a.icao24}>
                                        <button
                                            onClick={() => handleAircraftClick(a.icao24)}
                                        ...
                                        >
                                            <div className="flex items-center justify-between">
                                                ...
                                                <span className="text-xs text-slate-400">{a.count} pts</span>
                                            </div>
                                            <div className="text-xs text-slate-400 mt-0.5">
                                                {fmt(a.firstSeen)} — {fmt(a.lastSeen)}
                                            </div>
                                        </button>
                                    </li>
                                ))}
